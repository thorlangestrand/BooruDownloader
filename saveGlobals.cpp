...
bool saveGlobals()
{
    ...
    std::string outFile(workablePath + "\\config.json");
    ...
    root.addPureData(globals::keyNameMap[globals::saveKeys::r34KeyEnum], globals::r34Key.c_str());
    root.addPureData(globals::keyNameMap[globals::saveKeys::r34UserIdEnum], globals::r34UserId.c_str());
    ...
    scuff::saveToFile(outFile.c_str(), root);

    root.erase(); // Always clean up after yourself!
    return true;

    // Same as with in initGlobals I'll probably keep this here for the foreseeable future
    //
