...
public:
    ...
    json(bool _array, const char *_label);
    json(bool _array, bool root);
    json(std::string _label, std::string _data, bool _str);
    json(const char *_label, int data);
    json(const char *_label, bool data);
    ...
    void addNode(std::string name, json *object);
    void addPureData(const char *name, const char *data);
    void addPureData(int, const char *data);
    void addPureData(const char *name, int data);
    void addPureData(int, int data);
    void addPureData(const char *name, bool data);
    void addPureData(int, bool data);

    // I'm not going to add the double type here
    // Half because I cba, half because there's really no point atm
    ...
    friend std::ostream &operator<<(std::ostream &out, const json &scope)
    {
        ...
        else if (scope.array)
        {
            ...
            {
                if (i != scope.nChildren - 1)
                {
                    scope.arrChildren[i]->print(out, depth + 1, true);
                }
                else
                {
                    scope.arrChildren[i]->print(out, depth + 1, true, true);
                }
            }
    ...
    void print(std::ostream &out, size_t depth, bool arrmember = false, bool finalArrMember = false)
    {
        ...
        else if (array)
        {
            ...
            for (size_t i = 0; i < arrChildren.size(); ++i)
            {
                if (i != nChildren - 1)
                {
                    arrChildren[i]->print(out, depth + 1, true);
                }
                else
                {
                    arrChildren[i]->print(out, depth + 1, true, true);
                }
            }
            for (size_t i = 0; i < depth; ++i)
            {
